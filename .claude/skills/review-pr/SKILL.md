...
Record: which areas the PR touches (the root site, `js/`, `data/`, `web-projects/`, `scripts/`), the total scope (additions/deletions, file count), the stated intent (commits + PR description), and any implementation proposal from the issue discussion.
...
3. **Quality, safety & security.** Explicit types; error handling (no silently swallowed errors); security (injection, path traversal, secrets in logs/bundles, missing auth on new endpoints, unsanitized user input); config and env vars documented; no dead code, no over-engineering, no unnecessary duplication.
4. **Testing & gaps.** Is red-green TDD respected (do the new tests assert real behavior, not just run the code)? What edge cases are untested? Name 2-3 realistic untested inputs and what would happen for each. Any cross-codebase impact.

Also spawn, as siblings (they cannot run inside the analysis agents):

- **validate** (always): run the repo's checks (format, lint/typecheck, tests, build) for the touched areas and report each as pass or fail, with failing output verbatim.
- **pattern-scout** (only when the diff adds a new web project, JS module, or data-driven section): find how similar ones are built here and report the convention with real examples.
...
4. **Classify severity.**
   - `[Required]`: blocks merge. Bugs that produce wrong output, security holes, missing deliverables from the issue, broken tests, type errors.
...
- **Findings index**: a numbered list of every finding that survived phase 4, so the user sees the total before the questions begin. Example:
  ```
  1. [Required]   Rows silently dropped on create - src/repo/userRepo.ts:58 - fix in this PR
  2. [Suggestion] Missing test for empty input - tests/user.test.ts - fix in this PR
  3. [Suggestion] Pre-existing: retry config duplicated - follow-up issue
  ```
...
Build one review payload and post it in one API call. Write the review body and every comment in the repo's **Communicating with users** style (`AGENTS.md`): the author skims them to decide what to change, so lead with the point and keep each one short and concrete. In `autonomous` mode, print one line first: "Posting autonomous verdict <EVENT> without human review, as requested via --autonomous."
