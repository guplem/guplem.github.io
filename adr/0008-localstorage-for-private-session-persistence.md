# ADR 0008: localStorage for Private-Session Persistence in Web-Projects
...
- A **user-facing reset** (button) clears the key, and the stored format is minimal — `rps-mind-reader` persists only the round list and cumulative totals, then *replays* them to rebuild the AI model (see ADR 0009), rather than serializing the model object itself.
