...
- **ai-world-gen** -- A map generator with two kinds of AI, started at the AI Hackathon Barcelona 2026. A text model writes a world's vocabulary once (types with placement rules, flags, a visual tag and their own `instanceFields`); a decision model (TypeSafe's Jev, through OpenRouter's `alpha/decisions` endpoint) then places one cell at a time from its neighbours, live on a canvas drawn from the Urizen 1-bit tileset. Five generation orders behind one `nextCoordinate(placed)` interface, a flood fill that reports sealed-off regions, an inspector with each cell's probabilities, and a JSON save. Bring-your-own OpenRouter key, kept in this browser and sent to one host; `invariants.test.js` pins that, and `README.md` holds the phase roadmap and the open questions (TDD-covered pure modules, see ai-world-gen ADR 0001-0003 and root ADR 0006, 0007, 0012 and 0013)
