...
- **Personalisation**: change a cell's type by hand, ask the model again about one cell, load a saved world back.
- **Two downloads**: **Download JSON** saves the world as data, which the page loads back. **Download PNG** saves the whole map as an image in the style on screen, edge to edge. The zoom, the pan, the frames and the corner marks stay out of the file, so it is the map and not a screenshot.
...
| `wholeMap.js` | The whole map in one call: the prompt with the legend, the plan and the rules |
| `wholeMapLoop.js` | Sends that prompt, reads the rows back, and asks again when they are wrong |
...
| `batchLoop.js` | Sends the batches, reads the answers, and shrinks the budget when the endpoint refuses one |
...
AI_WORLD_GEN_GENERATION=whole-map python evaluate.py run --version v11-llm   # the whole map in one call, for comparison
AI_WORLD_GEN_GENERATION=batched python evaluate.py run --version v11-batch  # every cell as a question in one call, for comparison
AI_WORLD_GEN_GENERATION=batched python evaluate.py run --version v4-batch --code-of v4  # the same, with v4's own code
```
...
| v1-llm | Not a version of the loop: the whole map in one call, with only what v1's loop gave Jev (v1's vocabulary, no target shares, no plan; `--code-of v1`) | 0.92 | 0.75 | 0.87 | 0.65 | n/a | 0.54 | 0.91 | 0.68 |
| v4-llm | Not a version of the loop: the whole map in one call, with only what v4's loop gave Jev (v4's vocabulary and target shares, no plan; `--code-of v4`) | 0.91 | 0.79 | 0.89 | 0.66 | n/a | 0.51 | 0.93 | 0.68 |
| v7-llm | Not a version of the loop: the whole map in one call, with only what v7's loop gave Jev (v7's vocabulary and typed rules, no plan; `--code-of v7`) | 0.92 | 0.76 | 0.90 | 0.62 | 0.80 | 0.65 | 0.90 | 0.68 |
| v8-llm | Not a version of the loop: the whole map in one call, with only what v8's loop gave Jev (v8's vocabulary, rules and blueprint; `--code-of v8`) | 0.94 | 0.76 | 0.82 | 0.78 | 0.76 | 0.65 | 0.84 | 0.65 |
| v11-llm | Not a version of the loop: the whole map in one call to Claude Sonnet 5 (thinking off), with v11's vocabulary, blueprint and rules in the prompt (`wholeMap.js`) | 0.94 | 0.75 | 0.85 | 0.74 | 0.76 | 0.61 | **0.90** | 0.65 |
| v1-batch | Not a version of the loop: v1's own per-cell questions, sent together (`--code-of v1`) | 0.58 | 0.61 | 0.79 | 0.45 | n/a | 1.00 | 0.21 | 0.20 |
| v4-batch | Not a version of the loop: v4's own per-cell questions, sent together (`--code-of v4`) | 0.78 | 0.55 | 0.79 | 0.38 | n/a | 0.79 | 0.70 | 0.32 |
| v7-batch | Not a version of the loop: v7's own per-cell questions, sent together (`--code-of v7`) | 0.64 | 0.57 | 0.80 | 0.39 | 0.94 | 0.64 | 0.47 | 0.25 |
...
- **v4: lines.** With the continuation judged by code, walls form lines and closed shapes and routes mostly join up. But three doors in four stood in open ground (door in a wall 0.26), walls came as filled blocks, and no map had a room you could enter.
...
- **v11-llm: the whole map in one call.** The measured alternative ADR 0002 had rejected on reasoning alone. With the same plan in its prompt, room by room, the text model still closed a room on only 47% of the maps (v11: 92%) and set 60% of its doors in a wall (87%); the typed rules held at 0.76 (0.99), because nothing stops a model writing 256 letters from breaking one. It won on landmarks (present 0.76 against 0.45), and it was fast: 3.7 s for an 8 × 8 map against 22, 6 s for 16 × 16 against 90. The run cost the same ($0.25 against $0.24); 8 of 38 maps needed a second answer. Left with thinking on, the model spent its whole 6,000-token budget reasoning and answered nothing.

- **v11-batch: the same decisions, sent together.** Jev answers a whole map of independent typed questions in one request: 3,008 decisions in 49 answered requests (one more was refused for its size), $0.086 against $0.24, and 40 s of drawing against 1,046 s. An 8 x 8 map is one request, 0.85 s and $0.0018; a 16 x 16 map does not fit and takes 4 or 5. Nothing fell back. What the code decides survived (structures 0.97, zone 1.00, doors in walls 0.87 to 0.94, an enclosed room on 0.95 of maps) and what needed a neighbour did not: 163 broken "never next to" pairs (181 cells), every one of them between two cells of the same batch (v11: 13 pairs, 11 cells); a type whose rules say "exactly one" more than once on 7 of 38 maps (v11: 0), worst a mansion with 39 staircases; path continuity 0.58 to 0.26; coverage 0.77 to 0.43. Two scores rose because the maps repeat themselves: ground in patches 0.65 to 0.83, one walkable region 0.72 to 0.93. The judge reads it at 0.57 against 0.69.
- **One call and batched at every stage (v1, v4, v7, v8 and v11).** Each of the two ran again with the code of four older versions, knowing only what that version gave Jev (`run --code-of vN`, `pastVersionVariants.js`). The one call was about as good from the start: the judge gives 0.68 at v1-llm and 0.65 at v11-llm, where the loop went from 0.09 to 0.69. Only rules about where things go moved it: the typed rules at v7 (routes to doors 0.51 to 0.65) and the plan at v8 (a place reads as a place 0.62 to 0.78). The batch rose with the loop only where a fix is code applied to each cell: the judge reads 0.20, 0.32 and 0.25 before the blueprint and 0.57 from v8 on, because the fixes of v2 to v7 all read cells decided before, and a batch has none. Before v8 every cell is offered every type, so an 8 × 8 batch takes two requests (54 and 10 cells). The new batch code, run at v11 against `v11-batch`, lands within 0.05 on every specification, so each point is read with that much noise. `rules` is n/a for v1 and v4: their vocabularies have no typed rules, and a variant is scored against its own version's vocabulary.
...
Every non-visual module has a sibling `*.test.js`, written first (root ADR 0012). `render.js`, `app.js` and `openRouterClient.js` have none by design; `batchLoop.js` and `wholeMapLoop.js` are tested through the sibling of the file that re-exports them.
