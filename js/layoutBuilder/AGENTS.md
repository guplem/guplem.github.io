...
- **Three-state chips (root ADR 0014):** every type and skill chip cycles none -> include -> exclude -> none. The state is a value in `data-tag-filter` (absent while the chip filters nothing), never the old boolean `selected` attribute: CSS matches `[selected]` by presence, so a value in it still applies the include look. Paint a chip only through `paintTagFilterButton()`, which the filter rows and the card chips share, and repaint after every state change with `syncFilterControls()`.
- **The skill-exclusion button:** `#myWorkExcludeToggle` in `index.html` names its target skill in `data-tag-id` (an `idFromText()` id, e.g. `VibeCoded`) and its display name in `data-tag-label`. `enableTagExclusionToggle()` keeps it hidden while no project carries that skill, so a renamed skill leaves a hidden button instead of a dead one.
