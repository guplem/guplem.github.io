...
Build one review payload and post it in one API call. Write the review body and every comment under the **Writing style** section of `AGENTS.md`: the author skims them to decide what to change, so lead with the point and keep each one short and concrete. In `autonomous` mode, print one line first: "Posting autonomous verdict <EVENT> without human review, as requested via --autonomous."
