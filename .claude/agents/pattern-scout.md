...
- **Filter and search contracts**: filter state lives in `workFilters.js` (`includedWorkTypes`, `excludedWorkTypes`, `includedWorkSkills`, `excludedWorkSkills` arrays; `workSearchQuery` via `getWorkSearchQuery`/`setWorkSearchQuery`). Every chip cycles none -> include -> exclude -> none and carries its state in `data-tag-filter` (root ADR 0014); `paintTagFilterButton()` paints every chip, in the filter rows and on the cards. Button matching normalizes with `idFromText()`; free-text search uses raw lowercase substring (`workMatchesText`) and deliberately does NOT use `idFromText()`.
- **Web-projects structure**: each folder under `web-projects/` is fully self-contained (own HTML/CSS/JS, no shared dependencies) except the data-driven `web-projects/index.html` directory index (root ADR 0008). Pure logic goes in its own module with a sibling `*.test.js`. See `web-projects/AGENTS.md`.
- **Test structure**: Bun test runner (`bun test`), no config or `package.json`. Test files sit next to source as `name.test.js`. Only pure logic is tested; visual/DOM rendering is exempt. New behaviour is written test-first, red-green (root ADR 0012).
- **Generated SEO mirror rule**: renderers that have a static fallback (`buildHeroHtml`, `buildWebProjectsIndexHtml` in `scripts/generateSeoBlocks.js`) must mirror their JS counterpart exactly (root ADR 0010). Changing one side without the other breaks silently.
