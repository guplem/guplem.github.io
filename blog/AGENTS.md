...
- **Check an animation by pinning it, not by waiting.** A hidden browser pane runs no clock, and headless Chrome's `--virtual-time-budget` skips frames of animations that change in steps, so both show wrong frames. Load the post in a temporary same-origin page that calls `pause()` and sets `currentTime` on every animation of the post, screenshot that page, then delete it. Pinning proves the design but hides drift, because it sets every animation to the same moment. So put two things that must stay in step (the fill loop's outline and tile) in one keyframe rule on one element: a second, delayed animation drifted behind in real browsers.
- **In the AI World Gen post, the three methods have one name, one colour and one shape each, everywhere.** Cell by cell: orange, solid line, circle. Batched: blue, dashed line, diamond. LLM: green, dashed line, square. Wrap every mention in the text in `.m-cell`, `.m-batch` or `.m-llm` (an underline in its colour, shown on hover), and keep "the loop" for the evaluation loop only. The Galtea run codes (`v11-llm`, `v11-batch`) stay as they are.
- **A contents list is a `<details class="toc" id="contents">` of plain links**, so it works with no script. A post may add a script that only opens it or marks the section being read (the AI World Gen post does both). Add every new `h2` to the list: `scripts/blogPosts.test.js` fails when one is missing.
