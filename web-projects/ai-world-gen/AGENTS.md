...
| `evaluation/evaluate.py` | No | The Galtea side: `setup`, `run --version vN`, `report`, `render`, `rescore`. Reads the rule names from `mapMetrics.js`; uploads each map's PNG and attaches it to the output; asks Galtea for the judge metric |
