...
const PATH = "web-projects/global-news-map";

/** The real catalogue, so a renamed key fails here too. */
const sayIn = (lang) => (key, params = {}) =>
  (MESSAGES[key]?.[lang] ?? key).replace(/\{(\w+)\}/g, (match, name) => (name in params ? String(params[name]) : match));
const say = sayIn("en");
...
describe("renderDeployLine", () => {
  ...
  test("writes the line in the language asked for", () => {
    const english = stubElement();
    const spanish = stubElement();
    renderDeployLine(english, STAMP, "en", say, escape, PATH);
    renderDeployLine(spanish, STAMP, "es", sayIn("es"), escape, PATH);
    expect(spanish.innerHTML).not.toBe(english.innerHTML);
    expect(spanish.innerHTML).toContain(">#82<");
  });
  ...
  test("leaves no untranslated key or unfilled slot, in either language", () => {
    for (const lang of ["en", "es"]) {
      for (const stamp of [STAMP, null]) {
        const element = stubElement();
        renderDeployLine(element, stamp, lang, sayIn(lang), escape, PATH);
        expect(`${lang}: ${element.innerHTML}`).not.toContain("ui.");
        expect(`${lang}: ${element.innerHTML}`).not.toContain("{");
      }
    }
  });
});
