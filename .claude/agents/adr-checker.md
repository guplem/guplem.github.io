...
1. Read all ADRs in `adr/`.
2. Read the ADR index in the root `CLAUDE.md` (Architecture Decision Records section).
...
1. Read all existing ADRs in `adr/`.
2. Determine if any new architectural decisions were made that are not yet recorded.
3. For each new decision, draft an ADR following the format: Context, Decision, Consequences.
4. Update the ADR index table in `CLAUDE.md`.
5. If an existing ADR is now superseded, note it in that ADR's file with a reference to the new one.

## ADR Format

File: `adr/NNNN-short-descriptive-title.md`
...
- Keep the index in `CLAUDE.md` in sync with the `adr/` directory.
