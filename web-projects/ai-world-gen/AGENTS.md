...
| `cellDecision.js` | Yes | `buildCellDecision` (state + one choice question), `balanceSheet` (target shares vs. counts), `continuationHints` (the code-judged suggestion), `readDecisionAnswer`, `chooseType` (sampling), the chat stand-in, `fallbackType` |
...
- **The state sent per cell never carries the grid.** It carries the 8-neighbours, counts within `NEARBY_RADIUS` (2), whole-map counts, the balance sheet and the continuation hints. `cellDecision.test.js` asserts the serialised state has no `cells` key. Keep it that way: a 24 × 24 grid per call multiplies the cost by the map size.
- **The code, not the model, judges structure.** `balanceSheet` reads each type's target share from the rarity word in its placement rules (`TARGET_SHARES`; the vocabulary prompt asks for exactly those words) and lists `needed` and `overused` types. `continuationHints` names one `suggested` type only for a closable gap or a line shorter than `MAX_CONTINUED_LINE` (4), never an overused type. v3 proved that asking the model to "continue the line" with no limit floods the map (see the README's version table); change the limits in code and measure, do not loosen the instruction.
