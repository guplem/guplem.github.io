...
//                          stored rounds rebuilds the exact model (see root ADR 0007).
...
export function learn(model, playerMove, aiMove) {
  ...
  // rebuildModel() reproduces the exact float state (see root ADR 0007).
  if (COUNT_DECAY < 1.0) {
...
// rounds, not the model (see root ADR 0007).
